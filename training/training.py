...
class Trainer:
    ...
    def __init__(
        ...
        # -inf so the first validation always counts as an improvement and a
        # best checkpoint is saved even when the initial metric is 0.
        self.best_metric = float("-inf")
        self.best_epoch = 0
        self.target_key = "mask" if task == "segmentation" else "label"
        self.metric_key = "mean_dice" if task == "segmentation" else "f1"

        # Per-epoch history for plotting training curves (e.g. in a notebook).
        self.history: dict[str, list] = {
            "epoch": [], "train_loss": [], "val_loss": [], self.metric_key: []
        }
    ...
    def _train_epoch(self, epoch: int) -> float:
        ...
        if self.progress:
            try:
                ...
                loader = tqdm(loader, desc=f"Epoch {epoch}", leave=False)
            except ImportError:
                pass
    ...
    def train(self) -> dict:
        ...
        for epoch in range(1, self.max_epochs + 1):
            ...
            if epoch % self.val_interval == 0:
                ...
                self.history[self.metric_key].append(metric_value)
                ...
                if self.task == "segmentation":
                    ...
                    metric_str = f"{self.metric_key}={metric_value:.4f}"
                else:
                    # metric_key (f1) is already included in the per-metric list
                    metric_str = " | ".join(
                ...
                logger.info(
                    ...
                    f"{metric_str}{extra_str} | lr={current_lr:.2e}"
                )
